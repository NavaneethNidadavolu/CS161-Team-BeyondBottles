...
//        val manager = Lin(this,3)
    ...
    fun fetchData() {
        var response : Response<List<BlogItem>>

        lifecycleScope.launch {
            response = getBlogs(type)
            if (response.isSuccessful) {
                Log.v("feed data", response.body().toString())
                response.body()?.let { mBlogAdapter!!.setData(it) }
            }
        }
    }
...
//        listBlogs.add(6, blogElement!!)
    ...
    private suspend fun getBlogs(type : String)
            = BaseProvider.api.getBlogs(type)
