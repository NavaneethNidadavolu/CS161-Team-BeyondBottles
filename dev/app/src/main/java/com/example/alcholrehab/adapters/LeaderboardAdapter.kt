...
class LeaderboardAdapter(private var blogList: List<LeadershipModel>) :
    ...
    override fun onBindViewHolder(holder: LeaderboardViewHolder, position: Int) {
        ...
        holder.textScore.text = "Score: " + currentItem.score
        loadImage(holder.ivUserAvatar)
    }

    override fun getItemCount(): Int = blogList.size

    private fun loadImage(imageView: ShapeableImageView) {
        val imageId = (Math.random() * images.size).toInt()
        imageView.setImageResource(images[imageId])
    }

    fun setData(it: List<LeadershipModel>) {
        blogList = it
...
//class LeaderboardAdapter(listBlogs: ArrayList<LeadershipModel>) : RecyclerView.Adapter<LeaderboardAdapter.LeaderboardViewHolder>(),
//    Serializable {
//
